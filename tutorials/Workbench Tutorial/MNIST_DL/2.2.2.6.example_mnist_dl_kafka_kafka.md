# MNIST를 이용한 딥러닝 모델 예제 (kafka방식으로 입력받아 kafka로 전달하기) 
...
예제 파일 "kafka_consumer.py"을 이용하여, kafka의 consumer를 실행시킵니다.  워크플로우를 실행시키고, 입력데이터가 전달되면, 예측 결과 값을 kafka의 mnist _output_dl_kafka 토픽을 통해 가져옵니다.

> //실행방법
>
> \>python kafka_consumer.py --ip csle1 --port 9092 --groupid mnist_output --topic mnist_output_dl_kafka
