...
1. 선택된 파일을 마우스 우클릭하여 **x 삭제** 버튼을 선택하면 해당 파일이  Local File System 리스트에서 제거됩니다.
2. 선택된 파일을 마우스 우클릭하여 **Local 파일 초기화**를 선택하면 Local File System 리스트에 존재하는 모든 파일이 제거됩니다.

<br>

## Swift Repository

서비스를 운용하기 위하여 (주)인투와이즈 관리자가 제공한 model, pymodule, dataset의 저장소입니다. 사용자의 실수 등으로 인하여 HDFS에 업로드 된 model, pymodule, dataset 등의 파일이 삭제되었을 경우 Swift Repository에 저장된 파일을 다운로드하여 복원할 수 있습니다. 사용자는 Swift Repository에 업로드 된 파일을 임의로 삭제할 수 없습니다.
...
2. HDFS에서 파일을 저장하고자 하는 경로를 선택하고,  **>** 버튼을 이용하면 선택한 HDFS 경로 밑으로 Swift Repository에서 선택된 파일/폴더가 이동합니다. 



<img src="./images/2.4.swift_share_list.png" alt="swift_share" style="zoom:50%;" />

1. Swift Repository 하단 share에 마우스 우클릭 후, **경로보기**를 선택하면 share의 경로가 표출됩니다. 아래의 화면과 같이 share의 경로가 표출됩니다.

   ![swifit_share_dir_ok](./images/2.4.swift_share_dir_ok.png)

2. Swift Repository 하단 share에 마우스 우클릭 후, **새로고침**을 선택하면 share가 새로고침 됩니다.

3. Swift Repository 하단 share에 마우스 우클릭 후, **Repository 선택 초기화**를 선택하면 선택이 해지됩니다.

4. Swift Repository 하단 share에 마우스 우클릭 후, **삭제**를 선택하면 "파일을 삭제하시겠습니까?" 팝업 창이 뜨고 **확인**을 선택하면 삭제됩니다.
...
4. HDFS 내 파일을 선택하고 마우스 우클릭 후, **이름변경**을 선택하면 파일 명을 변경할 수 있습니다. 아래 화면은 파일 명을 변경할 경우 표출되는 팝업 창입니다.
